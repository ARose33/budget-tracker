...
const operationPlan =
  ["export", "review"].includes(operation) ||
  (operation === "dry-run" && argument("snapshot") && argument("statements"))
    ? null
    : requireExistingDataOperation("statements:" + operation, {
...
function total(rows) {
  return (
    Math.round(
      rows.reduce((sum, row) => sum + Number(row.amount ?? 0), 0) * 100,
    ) / 100
  );
}
...
function summaryTable(rows) {
  ...
  return [
    ...
    ...rows.map(
      (row) =>
        `| ${row.year} | ${row.account} | ${row.count} | ${money(row.totalAmount)} |`,
    ),
  ].join("\n");
}
  ...
  for (const decision of manifest.insertionDecisions) {
    ...
    if (decision.decision === "approved") {
      ...
      entry.approvedAmount =
        Math.round((entry.approvedAmount + Number(row.amount)) * 100) / 100;
    } else {
      entry.pending += 1;
      entry.pendingAmount =
        Math.round((entry.pendingAmount + Number(row.amount)) * 100) / 100;
    }
...
function markdownReport(report) {
  const mapped = report.accountMappings.filter(
    (mapping) => mapping.status === "mapped",
  );
  const review = report.accountMappings.filter(
    (mapping) => mapping.status !== "mapped",
  );
  const sections = [
    ...
    `- Statement transactions parsed: ${report.inventory.parsedTransactions} (${money(
      total(
        report.missingFromSupabase.concat(
          report.exactMatches,
          report.highConfidenceMatches,
          report.possibleMatches,
          report.conflicts,
    ...
    ...report.inventory.folders.map(
      (folder) =>
        `| ${folder.folder} | ${folder.files} | ${folder.transactions} |`,
    ),
    "",
    "## Account mappings",
    "",
    ...mapped.map((mapping) => {
      const account = mapping.candidates.find(
        (candidate) => candidate.id === mapping.accountId,
      );
    ...
    ...(review.length > 0
      ? [
          ...
          ...review.map(
            (mapping) =>
              `- ${mapping.statementAccountKey}: ${mapping.rationale} Candidates: ${
                mapping.candidates
...
async function exportReport(report, outputDirectory) {
  ...
  await Promise.all([
    ...
    writeCsv(
      path.join(outputDirectory, "proposed-insertions.csv"),
      report.proposedInsertions,
    ),
    writeCsv(
      path.join(outputDirectory, "exact-matches.csv"),
      report.exactMatches,
    ),
    writeCsv(
    ...
    writeCsv(
      path.join(outputDirectory, "possible-matches.csv"),
      report.possibleMatches,
    ),
    writeCsv(
      path.join(outputDirectory, "likely-duplicates.csv"),
      report.likelyDuplicates,
    ),
    ...
    writeCsv(
      path.join(outputDirectory, "parsing-failures.csv"),
      report.parsingFailures,
    ),
    writeCsv(
      path.join(outputDirectory, "unmapped-accounts.csv"),
      report.unmappedAccounts,
    ),
    writeCsv(
      path.join(outputDirectory, "unmapped-transactions.csv"),
      report.unmappedTransactions,
    ),
    writeCsv(
      path.join(outputDirectory, "statement-coverage.csv"),
      report.statementCoverage,
    ),
    writeCsv(
      path.join(outputDirectory, "statement-files.csv"),
      report.statementFiles,
    ),
  ]);
}
...
async function dryRun() {
  ...
  const statementsRoot = path.resolve(
    argument("statements") ?? defaultStatementsRoot,
  );
  const outputDirectory = path.resolve(
    argument("output") ?? path.join(defaultOutputRoot, "latest"),
  );
  const throughDate =
    argument("through") ?? new Date().toISOString().slice(0, 10);
  const manifestPath = argument("manifest");
  const reviewManifest = manifestPath
    ? JSON.parse(await fs.readFile(path.resolve(manifestPath), "utf8"))
    : null;
  const parsed = await parseAllStatements(
    statementsRoot,
    ({ index, total: count, filePath }) => {
      console.log(
        `[${index}/${count}] ${path.relative(statementsRoot, filePath)}`,
      );
    },
  );
  console.log("Loading Supabase accounts and transactions (read-only)...");
  const snapshot = snapshotPath
    ? JSON.parse(await fs.readFile(path.resolve(snapshotPath), "utf8"))
    ...
    JSON.stringify(
      {
        ...
        reconciliationInsertions:
          report.reconciliationInsertions?.length ??
          report.proposedInsertions.length,
...
async function applyReport() {
  ...
  if (!reportPath)
    throw new Error("--report=<path-to-reconciliation.json> is required.");
  const manifestPath = argument("manifest");
  if (!manifestPath)
    throw new Error("--manifest=<path-to-review-manifest.json> is required.");
  ...
  console.log(
    JSON.stringify(
      { batchId: result.batchId, inserted: result.inserted, receiptPath },
      null,
      2,
    ),
  );
...
async function prepareReview() {
  ...
  const outputDirectory = path.resolve(
    argument("output") ?? path.dirname(path.resolve(reportPath)),
  );
  await fs.mkdir(outputDirectory, { recursive: true });
  ...
  await Promise.all([
    ...
    writeCsv(
      path.join(outputDirectory, "insertion-decisions.csv"),
      manifest.insertionDecisions,
    ),
    writeCsv(
      path.join(outputDirectory, "account-decisions.csv"),
      manifest.accountDecisions,
    ),
    writeCsv(
      path.join(outputDirectory, "date-decisions.csv"),
    ...
    JSON.stringify(
      {
        ...
        pendingAccounts: manifest.accountDecisions
    ...
    JSON.stringify(
      {
        mode: "review",
        approvalToken: validation.expectedToken,
        manifestTokenIsCurrent:
          manifest.approvalToken === validation.expectedToken,
        totals: validation.totals,
...
async function verifyReceipt() {
  ...
  const receipt = JSON.parse(
    await fs.readFile(path.resolve(receiptPath), "utf8"),
  );
  const env = await loadEnvironment(repoRoot);
  assertOperationTarget(operationPlan, env.NEXT_PUBLIC_SUPABASE_URL);
...
async function exportExistingReport() {
  ...
  console.log(
    JSON.stringify(
      { mode: "export", reportId: report.reportId, outputDirectory },
      null,
      2,
    ),
  );
