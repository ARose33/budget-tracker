...
const operationPlan = requireExistingDataOperation("statements:seed-review", {
  write: true,
});
...
function parseEnv(text) {
  return Object.fromEntries(
    text.split(/\r?\n/).flatMap((line) => {
      ...
      if (!match) return [];
      return [
        [match[1].trim(), match[2].trim().replace(/^(['"])(.*)\1$/, "$2")],
      ];
    }),
  );
}
...
if (report.reportId !== manifest.reportId)
  throw new Error("Report and review manifest do not match.");
...
const decisions = new Map(
  manifest.insertionDecisions.map((item) => [item.sourceRecordId, item]),
);

function toReview(item, reviewType) {
  const candidateIds =
    item.candidateSupabaseTransactionIds ??
    (item.matchedSupabaseTransactionId
      ? [item.matchedSupabaseTransactionId]
      : []);
  const proposed = report.proposedInsertions.find(
    (row) => row.sourceRecordId === item.sourceRecordId,
  );
  ...
  return {
    ...
    account_id: item.mappedAccountId,
    account_name:
      accountNames.get(item.mappedAccountId) ?? insertRecord.account ?? null,
    ...
    reason:
      item.reason ??
      (reviewType === "proposed_import"
        ? "Not matched to an existing Supabase transaction."
        : null),
...
for (let offset = 0; offset < rows.length; offset += 200) {
  const { error } = await supabase
    ...
    .upsert(rows.slice(offset, offset + 200), {
      onConflict: "user_id,report_id,source_record_id,review_type",
      ignoreDuplicates: true,
    });
  if (error) throw error;
}

// Existing reports and decisions are never superseded automatically.
...
console.log(
  JSON.stringify({ proposed: rows.length, pending: count, ledgerWrites: 0 }),
);
