...
function collect(directory) {
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    return /\.(test|spec)\.(ts|mjs)$/.test(name) && !name.includes(path.sep + "e2e" + path.sep) ? [name] : [];
  });
}
const files = [...collect(path.join(root, "src")), ...collect(path.join(root, "tests")),
  "scripts/lib/statement-reconciler.test.mjs", "scripts/lib/statement-review.test.mjs"];
...
process.exitCode = await runNode(["--import", "./scripts/test-network-guard.mjs", "--test", "--test-concurrency=1", ...files, ...process.argv.slice(2)]);
