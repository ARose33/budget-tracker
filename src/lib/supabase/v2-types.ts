...
export type Database = Omit<Baseline, "public"> & {
  public: Omit<Baseline["public"], "Tables" | "Functions"> & {
    Tables: Omit<
      Tables,
      "transactions" | "budgets" | "bank_connections" | "accounts"
    > & {
      accounts: ExtendTable<
        Tables["accounts"],
        { bank_balance_managed: boolean }
      >;
      sync_receipts: {
        Row: {
          operation_id: string;
      ...
      bank_connections: ExtendTable<
        Tables["bank_connections"],
        {
    ...
    Functions: Baseline["public"]["Functions"] & {
      ...
      stackmint_transaction_history: RPC<{
        p_id: string;
        p_kind?: string;
        p_page?: number;
      }>;
      stackmint_legacy_connections: RPC<Record<string, never>>;
      stackmint_reconciliation: RPC<{
        p_filters?: Json;
        p_summary_only?: boolean;
      }>;
      stackmint_resolve_reconciliation: RPC<{
        p_id: string;
        p_version: number;
        p_decision: string;
        p_candidate_id?: string | null;
        p_candidate_version?: number | null;
        p_note?: string | null;
      }>;
      stackmint_begin_categorization: RPC<{ p_id: string; p_token: string }>;
      stackmint_finish_categorization: RPC<{
        p_id: string;
        p_token: string;
        p_items?: Json;
      }>;
      ...
      stackmint_resolve_provider_review: {
        Args: {
          p_id: string;
          p_decision: string;
          p_expected_version?: number | null;
          p_expected_balance?: number | null;
        };
        Returns: undefined;
      };
      stackmint_begin_disconnect: {
        Args: { p_connection_id: string; p_user_id: string };
        Returns: boolean;
      };
      stackmint_release_sync: {
        Args: { p_connection_id: string; p_token: string; p_error?: string };
        Returns: undefined;
      };
      stackmint_apply_sync: RPC<{
        p_connection_id: string;
        p_token: string;
        p_generation: number;
        p_base_cursor: string | null;
      ...
      stackmint_save_split: RPC<{
        p_parent_id: string;
        p_expected_version: number;
        p_allocations: Json;
      }>;
      stackmint_unsplit: RPC<{
        p_parent_id: string;
        p_expected_version: number;
      }>;
      stackmint_category: { Args: { p_command: Json }; Returns: string };
      stackmint_write_note: {
        Args: {
          p_user_id: string;
          p_transaction_id: string;
          p_expected_version: number;
          p_legacy_content: string;
          p_content: string;
        };
        Returns: number;
      };
