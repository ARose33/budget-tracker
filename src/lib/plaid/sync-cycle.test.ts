...
const page = (next_cursor: string, has_more = false): SyncPage => ({
  accounts: [],
  changed: [],
  removed: [],
  next_cursor,
...
function dependencies() {
  ...
  const deps: SyncDependencies = {
    acquire: async () => ({
      token: "synthetic-operation",
      generation: 1,
      cursor: "original",
    }),
    accounts: async () => [],
    page: async () => page("finished"),
    apply: async (_lease, _accounts, _changed, _removed, cursor) => {
      applied.push(cursor);
      return result;
    },
    receipt: async () => null,
    release: async (_lease, error) => {
      released.push(error);
    },
  };
...
test("a failed pagination loop has no ledger or cursor writes", async () => {
  ...
  assert.deepEqual(released, ["SYNC_INCOMPLETE"]);
});
test("pagination mutations restart the full loop at the original cursor before one atomic commit", async () => {
  ...
  deps.page = async (cursor) => {
    ...
    if (cursor === "original") return page("second", true);
    if (calls.length === 2)
      throw { errorCode: "TRANSACTIONS_SYNC_MUTATION_DURING_PAGINATION" };
