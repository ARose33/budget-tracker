  ...
  try {
    ...
    while (true) {
      const accounts = new Map(
        initialAccounts.map((account) => [account.provider_id, account]),
      );
      const changed: ProviderTransaction[] = [],
        removed: string[] = [];
      ...
      try {
        for (let pages = 0; ; pages++) {
          if (now() - start > 45_000)
            throw new Error(
              "Sync timed out before committing. Retry this connection.",
            );
          if (pages >= 100)
            throw new Error(
              "This provider cycle exceeds the supported size. No cycle data was committed.",
            );
          ...
          for (const account of page.accounts)
            accounts.set(account.provider_id, account);
          changed.push(...page.changed);
          removed.push(...page.removed);
          if (changed.length > 50_000 || removed.length > 50_000)
            throw new Error(
              "Provider cycle is too large. No cycle data was committed.",
            );
        ...
        if (
          errorCode(error) === "TRANSACTIONS_SYNC_MUTATION_DURING_PAGINATION" &&
          retries++ < 2
        )
          continue;
        throw error;
    ...
    await deps.release(lease, failure).catch(() => {
      /* The lease expires; preserve the proven ledger result or original failure. */
    });
  }
}
