...
const rowSchema = z.object({
  id: z.string(),
  entity_type: z.enum(["account", "transaction"]),
  entity_id: z.string().nullable(),
  reason: z.string(),
  status: z.string(),
  created_at: z.string(),
  proposed_values: values,
  current: values.nullable(),
  existing_values: values.nullable(),
  candidates: z.array(
    z.object({
      id: z.string(),
      date: z.string(),
...
export async function getProviderReviews(status: string, page = 0) {
  const { data, error } = await supabase.rpc("stackmint_provider_reviews", {
    p_status: status,
    p_page: page,
  });
...
export async function resolveProviderReview(
  ...
  const { error } = await supabase.rpc("stackmint_resolve_provider_review", {
    p_id: review.id,
    p_decision: decision,
    p_expected_version: review.current?.row_version,
