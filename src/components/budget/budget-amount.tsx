...
export function BudgetAmount({
      ...
      saveBudgets([
        {
          ...
          year,
          month,
          budget_limit: parseAmount(draft),
  ...
  if (!editing)
    return (
      <Button
        variant="ghost"
        className="h-auto min-h-10 px-2 tabular-nums"
        aria-label={
          "Edit " +
          item.line_item_name +
          " budget for " +
        ...
        onClick={() => {
          ...
          setDraft(item.budget_limit.toFixed(2));
          mutation.reset();
          setEditing(true);
        }}
      >
        {item.has_budget ? (
          ...
          <span className="text-muted-foreground">Set budget</span>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-end gap-1">
        <Input
          aria-label={item.line_item_name + " budget amount"}
      ...
      <p className="text-xs text-muted-foreground">
        {mutation.isPending ? "Saving…" : "This month only"}
      </p>
      {mutation.isError ? (
