  ...
  return (
    <div className="space-y-3 border-t pt-4">
      <Button variant="outline" size="sm" onClick={() => setOpen(!open)}>
        {open ? "Hide note" : "Open note"}
      </Button>
      {open ? <LoadedNote id={id} /> : null}
    </div>
  );
}
...
function NoteFields({ id, latest }: { id: string; latest: NoteState }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (saved) => {
      ...
      await client.invalidateQueries({ queryKey: ["transaction-note", id] });
      await client.invalidateQueries({ queryKey: ["transaction-history", id] });
    },
  });
  return (
    <form
    ...
    >
      ...
      <p className="text-xs text-muted-foreground">
        Previous versions are retained when a note is edited or cleared.
      </p>
      {latest.hash !== original.hash || latest.version !== original.version ? (
        <p role="alert" className="text-sm text-destructive">
          A newer note is available. Your draft is retained. Close and reopen
          the note to review it.
        </p>
      ) : null}
      {mutation.isError ? (
        <p role="alert" className="text-sm text-destructive">
      ...
      <Button
        size="sm"
        type="submit"
        disabled={mutation.isPending || draft === original.content}
      >
