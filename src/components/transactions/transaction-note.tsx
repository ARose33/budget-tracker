...
function LoadedNote({ id }: { id: string }) {
  ...
  return <NoteFields key={query.data.version + ":" + query.data.hash} id={id} original={query.data} />;
}
function NoteFields({ id, original }: { id: string; original: NoteState }) {
  const client = useQueryClient();
  const [draft, setDraft] = useState(original.content);
  const mutation = useMutation({
    mutationFn: () => updateTransactionNotes(id, draft, original),
    onSuccess: () => client.invalidateQueries({ queryKey: ["transaction-note", id] }),
  });
