...
## Proposals 09–11: review, receipts and accessible history

09 adds observed versions to statement reviews and journals future decisions. Original proposals and completed decisions cannot be overwritten. An owner-scoped aggregate replaces the 5,000-row summary limit; filtered pagination includes all decision history. Matching requires the currently observed candidate version and never edits the matched transaction. Explicit import requires a valid mapped account and source identity; an existing identity causes a conflict, never an inferred match. Original source payloads remain in the retained review. The old decision RPC is retained but execution is revoked.

10 adds immutable statement batch receipts and service-only import/archive commands. One bounded import and its full approved source payload commit together. Retrying a batch returns the durable result. Archive rejects later edits, splits, notes and review relationships, keeps every record, and can be undone through Transactions. Legacy receipts require individual review. No receipt or financial data is backfilled. These commands require separate existing-data authorization and verified recovery; see stackmint-v2-operations.md.

11 adds paginated owner-only note/change history, legacy connection metadata reads without credentials, and exact categorization queue counts. Archived split children remain accessible through their parent's history. Pre-v2 changes are not invented or reconstructed. Historical balances and opening values remain accessible in Accounts. Names shown for old category/account references use their current names; original IDs remain stored.

The complete proposal set has been applied only to disposable PostgreSQL fixtures. A test compares every original transaction field before/after all proposals, confirms no fabricated historical journal entries, and checks two-user access and token exclusion. The proposals still require staging permission/caller review; retain all additive structures during application rollback.
