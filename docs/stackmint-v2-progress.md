...
### Final release candidate checkpoint — September 9, 2026

- Completed verified cleanup, separate baseline diff review and synthetic desktop/mobile workflow checks. Review findings were corrected in 48b9783: frozen observed versions, retained bulk drafts, fresh initial editor/note reads, exact statement amounts, correct operator owner checks, safe API errors, deferred Plaid Link and same-origin auth redirects.
- Final commands: node scripts/test.mjs (58 passed, 0 failed/skipped); node scripts/isolated.mjs lint, typecheck and build (all exit 0; 29 generated routes); changed operator syntax checks and git diff --check (exit 0). Native TypeScript module-format warnings remain visible. No assertions were weakened.
- Browser checks completed: split/archive/restore and note-version clear history, exact category totals, mobile 390 × 844 layouts, all Analysis routes, explicit future-month copy and two-user sign-out/sign-in isolation. The earlier local-origin CORS mismatch was resolved by relative auth redirects. Screenshots contain synthetic fixtures only.
- Candidate version is 2.0.0-rc.1. Release notes, evidence, run commands, limitations and post-launch-write-preserving rollback procedure are in stackmint-v2-release-candidate.md; separate review in stackmint-v2-final-review.md. Runtime scope is complete for local candidate review. Existing-data schema execution, verified recovery/staging and production release still require separate approval. Nothing was pushed or deployed.
