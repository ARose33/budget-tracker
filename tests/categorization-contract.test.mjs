...
test("AI assignments cross the real PostgreSQL JSON contract, preserve prior categories, and isolate owners", async () => {
  ...
  try {
    ...
    const foreignCategory = (
      await db.query(
        "insert into public.budget_categories(group_name,line_item_name,category_type) values ('Other','Synthetic','Expense') returning id",
      )
    ).rows[0].id;
    await asUser(db, userA);
    const category = (
      await db.query(
        "insert into public.budget_categories(group_name,line_item_name,category_type) values ('Everyday','Synthetic','Expense') returning id",
      )
    ).rows[0].id;
    const transaction = (
      await db.query(
        "insert into public.transactions(date,description,amount) values ('2026-09-05','Synthetic purchase',-12) returning id",
      )
    ).rows[0].id;
    const apply = (items) =>
      db
        .query(
          "select public.apply_transaction_categorizations($1::jsonb) as changed",
          [JSON.stringify(items)],
        )
        .then((r) => Number(r.rows[0].changed));
    assert.equal(
      await applyCategorizationAssignments(
        [{ transactionId: transaction, categoryId: foreignCategory }],
        apply,
      ),
      0,
    );
    assert.equal(
      await applyCategorizationAssignments(
        [{ transactionId: transaction, categoryId: category }],
        apply,
      ),
      1,
    );
    ...
    const stored = (
      await db.query(
        "select id,category_id,categorization_status from public.transactions where id=$1",
        [transaction],
      )
    ).rows[0];
