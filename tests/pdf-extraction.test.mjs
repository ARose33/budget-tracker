...
test("real PDF extraction keeps page/row order and parses synthetic purchases and payments in the selected year", async () => {
  const doc = await extractPdfBytes(
    pdfFixture([
      [
        "CHASE Freedom Unlimited",
        "Opening/Closing Date 08/15/26 - 09/14/26",
        "08/20 08/21 SYNTHETIC GROCER 54.32",
      ],
      ["09/02 09/03 PAYMENT THANK YOU 100.00"],
    ]),
  );
  assert.equal(doc.pageCount, 2);
  assert.equal(doc.pages[1].pageNumber, 2);
  const result = parseStatementText(
    doc.pages.flatMap((page) => page.lines.map((line) => line.text)),
    { targetYear: 2026, accountTypeHint: "credit_card" },
  );
  assert.deepEqual(
    result.transactions.map((row) => ({ date: row.date, amount: row.amount })),
    [
      { date: "2026-08-20", amount: -54.32 },
      { date: "2026-09-02", amount: 100 },
    ],
  );
  assert.equal(
    parseStatementText(doc.text.split("\n"), {
      targetYear: 2023,
...
test("PDF extraction rejects invalid, oversized and overlong inputs, and reports a textless page without inventing rows", async () => {
  ...
  await assert.rejects(
    extractPdfBytes(new Uint8Array(MAX_PDF_BYTES + 1)),
    /15 MB/,
  );
  await assert.rejects(
    extractPdfBytes(pdfFixture(Array.from({ length: 101 }, () => []))),
    /100 pages/,
  );
